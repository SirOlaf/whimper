...
use super::ir::{IRBinOpKind, IRExpr, IRInst, NativeFlag, Program, VariableId, VariableType};
...
fn instruction(output: &mut String, instr: &IRInst, indent: usize, names: &[String]) {
    ...
    match instr {
        ...
        IRInst::If {
            ...
            instruction(output, then_branch, indent + 1, names);
            writeln!(output, "{padding}}} else {{").unwrap();
            instruction(output, else_branch, indent + 1, names);
