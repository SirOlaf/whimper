use super::{
    Context, Effects, IRBinOpKind, IRExpr, IRInst, Kind, LoopCondition, Value, VariableId,
    repeatable, visit,
};
...
#[derive(Default)]
pub(super) struct Collector(pub(super) Vec<LoopAnalysis>);

impl visit::Analyzer for Collector {
    fn instruction(&mut self, instr: &IRInst, _offset: usize, context: &Context) {
        if let Some(analysis) = analyze(instr, context) {
            self.0.push(analysis);
        }
    }
}
