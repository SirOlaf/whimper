//! A readable view of tier 4 for manual debugging.
...
fn precedence(expr: &IRExpr) -> u8 {
    match expr {
        ...
        IRExpr::BinOp { .. } => 6,
        IRExpr::Not(..) | IRExpr::Deref(..) | IRExpr::CastUnknownPtr { .. } => 7,
        _ => 8,
    }
}
...
/// Render the complete tier 4 program, including its entry and source offsets.
